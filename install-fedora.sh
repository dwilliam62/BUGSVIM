...
update_treesitter_cli() {
  ...
  if ! command -v tree-sitter >/dev/null 2>&1; then
    ...
    if sudo dnf install -y tree-sitter-cli 2>/dev/null; then
      echo -e "${GREEN}✓ tree-sitter-cli installed via dnf${NC}"
      ...
      echo -e "${RED}✗ Unable to install tree-sitter-cli (install dnf package or rust/cargo)${NC}"
    fi
  else
...
run_update_tasks() {
  echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║   bugsvim - Fedora Update Tasks                                ║${NC}"
  echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
  echo ""
...
check_and_install_deps() {
  ...
  echo -e "${BLUE}Installing missing dnf packages...${NC}"
  sudo dnf install -y \
    git \
    ripgrep \
    fd \
