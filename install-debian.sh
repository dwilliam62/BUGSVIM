...
update_treesitter_cli() {
  ...
  if ! command -v tree-sitter >/dev/null 2>&1; then
    ...
    if sudo apt-get install -y tree-sitter-cli 2>/dev/null; then
      echo -e "${GREEN}✓ tree-sitter-cli installed via apt${NC}"
      ...
      echo -e "${BLUE}Installing tree-sitter-cli via npm...${NC}"
      npm install -g tree-sitter-cli && echo -e "${GREEN}✓ tree-sitter-cli installed via npm${NC}"
    else
      echo -e "${RED}✗ Unable to install tree-sitter-cli (install apt package or rust/cargo)${NC}"
    fi
  else
...
run_update_tasks() {
  echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║   bugsvim - Debian/Ubuntu Update Tasks                         ║${NC}"
  echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
  echo ""
...
check_and_install_deps() {
  ...
  echo -e "${BLUE}Installing missing apt packages...${NC}"
  ...
  if command -v npm &>/dev/null; then
    ...
    local npm_pkgs=(bash-language-server @johnnymorganz/stylua-bin prettier @fsouza/prettierd vscode-langservers-extracted)
...
# Disabling neovikm install. It seems to break on updates
#  When installed here. Conflicts on two neovim pacakges
#  Trying to update sdame file
