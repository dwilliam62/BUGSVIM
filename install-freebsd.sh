...
ensure_neovim_supported() {
  ...
  if ! command -v nvim &>/dev/null; then
    log_info "NeoVim is not installed. Installing via pkg..."
    run_as_root pkg install -y neovim || true
  fi
  ...
  if [ "$major" -lt 10 ]; then
    ...
    log_warn "Please upgrade NeoVim: sudo pkg upgrade neovim"
    exit 1
  fi
...
check_and_install_deps() {
  echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║   bugsvim - Checking and Installing Dependencies (FreeBSD)     ║${NC}"
  echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
  echo ""
