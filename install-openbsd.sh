...
usage() {
  cat <<'EOF'
Usage: install-openbsd.sh [options]
...
update_treesitter_cli() {
  ...
  if ! command -v tree-sitter >/dev/null 2>&1; then
    ...
    if run_as_root pkg_add tree-sitter 2>/dev/null; then
      echo -e "${GREEN}✓ tree-sitter installed via pkg_add${NC}"
      ...
      echo -e "${RED}✗ Unable to install tree-sitter-cli (install pkg_add tree-sitter or rust/cargo)${NC}"
    fi
  else
...
run_update_tasks() {
  echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║   bugsvim - OpenBSD Update Tasks                               ║${NC}"
  echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
  echo ""
...
if ! command -v nvim &>/dev/null; then
  ...
  echo -e "${RED}Install NeoVim with: doas pkg_add neovim${NC}"
  exit 1
fi
...
if [ "$MAJOR" -lt 0 ] || ([ "$MAJOR" -eq 0 ] && [ "$MINOR" -lt 10 ]); then
  ...
  echo -e "${YELLOW}Please upgrade NeoVim: doas pkg_add -u neovim${NC}"
  exit 1
fi
...
if ! uname -s | grep -q "OpenBSD" 2>/dev/null; then
  echo -e "${YELLOW}Warning: This script is optimized for OpenBSD.${NC}"
...
echo -e "${BLUE}Step 1: Updating installed packages...${NC}"
run_as_root pkg_add -u
...
run_as_root pkg_add \
  ...
  gmake \
  pkgconf \
  tree-sitter || update_treesitter_cli
...
run_as_root pkg_add shfmt || echo -e "${YELLOW}Warning: shfmt not available${NC}"

echo -e "${BLUE}  Installing clang-format (via llvm)...${NC}"
run_as_root pkg_add llvm || echo -e "${YELLOW}Warning: llvm not available${NC}"

echo -e "${BLUE}  Installing stylua (via pkg) and prettier (via npm)...${NC}"
run_as_root pkg_add stylua || echo -e "${YELLOW}Warning: stylua not available${NC}"
...
echo -e "${YELLOW}Note: lua-language-server may need a manual install on OpenBSD${NC}"
echo -e "${YELLOW}Try: doas pkg_add lua-language-server${NC}"
...
echo -e "${BLUE}Step 9: Optional - Install lua-language-server via pkg_add${NC}"
read -p "Install lua-language-server? (y/n) " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
  if pkg_info -e lua-language-server >/dev/null 2>&1; then
    echo -e "${GREEN}✓ lua-language-server already installed${NC}"
  else
    if run_as_root pkg_add lua-language-server; then
  ...
  echo -e "${YELLOW}Skipping lua-language-server${NC}"
fi
